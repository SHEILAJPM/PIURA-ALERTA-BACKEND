...
| GET    | `/api/reportes-ciudadanos?limite=&conFoto=&antes=` | Feed de reportes, más recientes primero (paginado por cursor: `antes=<creado_en del último recibido>` trae la página siguiente). `conFoto=true` filtra solo los que tienen foto (fila de "historias" del frontend). Si hay sesión, cada reporte incluye `te_gusta` |
